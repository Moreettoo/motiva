            ...
            >
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="truncate text-sm font-medium text-ink">{c.email}</span>
                  <Chip tamanho="sm" icone={<IconeDominio nome={cargo.icone} />}>
                    {cargo.rotulo}
                  </Chip>
                  {c.equipe_nome ? (
                  ...
                  {venceu ? (
                    <Chip tamanho="sm" tom="warning" icone={<IconeDominio nome="TriangleAlert" />}>
                      expirado
                    </Chip>
                  ) : null}
                </div>
                <p className="mt-1 text-xs text-ink-3">
                  ...
                  {venceu ? `venceu em ${fmt.dataMedia(c.expira_em)}` : `vale até ${fmt.dataMedia(c.expira_em)}`}
                </p>
              </div>
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-2">
                <Botao
                  ...
                  iconeEsquerda={<Send />}
                  onClick={() =>
                    executar("Convite reenviado", () => reenviarConvite(c.id), {
                      aoConcluir: (dados) => {
                        setLinks((atual) => ({ ...atual, [c.id]: dados.link }));
                ...
                {confirmando === c.id ? (
                  <>
                    <Botao
                      ...
                      onClick={() => {
                        setConfirmando(null);
                        executar("Convite revogado", () => revogarConvite(c.id));
                      }}
                    >
                      Confirmar revogação
                    </Botao>
              ...
              {link ? (
                <Aviso tom="info" titulo="Link deste convite">
                  <p className="font-mono text-2xs break-all text-ink-2">{link}</p>
                </Aviso>
              ) : null}
            </div>
          );
